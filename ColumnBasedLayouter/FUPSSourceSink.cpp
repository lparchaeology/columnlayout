...
/** \file
 * \brief Implementation of FUPSSourceSink class.
 * 
...
Module::ReturnType FUPSSourceSink::doCall(UpwardPlanRep &UPR,		
		List<edge> &delEdges) 
{
	...
	/*
	//DEBUG
	GraphAttributes uga(UPR);
	UPRFourBendLayouter fbl;
	fbl.layout(UPR, uga);
	uga.writeGML("FUPSSourceSink::doCall_UPR.gml");
	*/
...
void FUPSSourceSink::computeFUPS(UpwardPlanRep &UPR, List<edge> &delEdges) 
{		
...
void FUPSSourceSink::getSpanTree(GraphCopy &GC, List<edge> &delEdges, bool random) 
{	
...
void FUPSSourceSink::dfs_visit(const Graph &G, 
						 edge e, 
...
bool FUPSSourceSink::constructMergeGraph(GraphCopy &M, adjEntry adj_orig, const List<edge> &orig_edges) 
{	
